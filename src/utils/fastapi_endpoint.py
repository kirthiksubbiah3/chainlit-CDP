...
def sentinelmind_api_get(  
    ...
    logger.info(f"Making GET request to {url} with params: {params} and headers: {headers}")    
    try:
...
def sentinelmind_api_post(  
    ...
    Returns:
        Union[Dict[str, Any], List[Any], str]: The JSON response from the API
            or an error message.
    """
    logger.info(f"Making POST request to {url} with data: {data}, json: {json_data}, headers: {headers}")
    response = httpx.post(url, data=data, json=json_data, headers=headers, timeout=100.0, follow_redirects=True)
    response_text = response.text

    logger.info(f"Response status code: {response.status_code} and response output is : {response_text}")
    ...
    try:
        json_data = [json.loads(line) for line in response_text.strip().splitlines() if line]
