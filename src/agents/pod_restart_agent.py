...
class EKSPodRestartModel(BaseModel):
    ...
    aws_role_arn: str = Field(
        description="Get the aws_role_arn from the AI message"
    )
    aws_profile: str = Field(
        description="Get the aws_profile from the AI message"
    )
    cluster_name: str = Field(
        description="Get the kubernetes cluster_name from the AI message"
    )
    region: str = Field(
        description="Get the aws region from the AI message"
    )
    namespace: str = Field(
...
class PodRestartAgent(BaseAgent):
    ...
    def run_saml_api(self, state: State):
        ...
        return {"messages": [
            self._to_assistant_msg(f"SAML API run for role {state['model'].aws_profile}")
        ]}
    ...
    def list_clusters(self, state: State):
        ...
        cmd = [
            ...
            "--region", state["model"].region
        ]

        result = subprocess.run(cmd, capture_output=True, text=True, check=True)
        return {"messages": [self._to_assistant_msg(f"List clusters\n {result.stdout}")]}
    ...
    def update_kubeconfig(self, state: State):
        ...
        cmd = [
            ...
            "--region", state["model"].region,
            "update-kubeconfig",
            "--name", state["model"].cluster_name,
            # "--profile", state["model"].aws_profile # TODO uncomment if profile available
        ]
        subprocess.run(cmd, check=True)
        return {"messages": [
            self._to_assistant_msg(f"Kubeconfig updated for{state['model'].cluster_name}")
        ]}
    ...
    def restart_deployment(self, state: State):
        ...
        cmd = [
            ...
            "-n", state["model"].namespace
        ]
        subprocess.run(cmd, check=True)
        return {"messages": [
            self._to_assistant_msg(f"Deployment {state['model'].deployment} restarted")
        ]}
    ...
    def verify_pods(self, state: State):
        ...
        result = subprocess.run(cmd, capture_output=True, text=True, check=True)
        return {"messages": [self._to_assistant_msg(f"Verifying pods\n {result.stdout}")]}
