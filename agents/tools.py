...
vector_store = Chroma(persist_directory=str(CHROMA_DB_DIRECTORY), embedding_function=embeddings)
...
def get_instructions(user_query: str) -> str:
    """
    Retrieves and VALIDATES the most relevant task instructions from the knowledge base.
    1. Retrieves the top document from ChromaDB.
    2. Uses an LLM to validate if the retrieved doc is relevant to the user's query.
    ...
    """
    print(f"TOOL (Stage 1 - Retrieval): Searching knowledge base for query: '{user_query}'")
    ...
    validation_prompt = ChatPromptTemplate.from_template(
        ...
        Does the RETRIEVED_DOCUMENT contain a direct plan to address the USER_QUERY? 
